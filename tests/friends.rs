...
fn test_add_friend_username() {
    block_on(async {
        ...
        let result = client.add_friends(&mut session1, &[], &["friendtestuser2"]).await;
        println!("{:?}", result);
...
fn test_add_friend_id() {
    block_on(async {
        ...
        let result = client.add_friends(&mut session1, &[&account2.user.id], &[]).await;
        println!("{:?}", result);
        assert_eq!(result.is_ok(), true);
    });
}
...
fn test_list_friend() {
    block_on(async {
        ...
        let friends = client.list_friends(&mut session1, None, Some(1), Some(&friends.cursor)).await.unwrap();
        println!("{:?}", friends);
        assert_eq!(friends.friends.len(), 1);
...
fn test_delete_friend() {
    block_on(async {
        let (client, mut session1, _, _) = helpers::clients_with_users("friendtestuser1", "friendtestuser2", "friendtestuser3").await;
        client.add_friends(&mut session1, &[], &["friendtestuser2"]).await.unwrap();
        let result = client.delete_friends(&mut session1, &[], &["friendtestuser2"]).await;
        println!("{:?}", result);
        assert_eq!(result.is_ok(), true);
    });
}
...
fn test_block_friend() {
    block_on(async {
        let (client, mut session1, _, _) = helpers::clients_with_users("friendtestuser1", "friendtestuser2", "friendtestuser3").await;
        let result = client.block_friends(&mut session1, &[], &["friendtestuser2"]).await;
        println!("{:?}", result);
        assert_eq!(result.is_ok(), true);
    })
}