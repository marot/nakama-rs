...
fn test_join_tournament() {
    block_on(async {
        let (client, mut session) = helpers::authenticated_client("tournamentclient1").await;
        let result = client.join_tournament(&mut session, "example-tournament").await;
        println!("{:?}", result);
...
fn test_list_tournaments() {
    block_on(async {
        let (client, mut session) = helpers::authenticated_client("tournamentclient1").await;
        let result1 = client.list_tournaments(&mut session, None, None, None, None, Some(1), None).await.unwrap();
        let cursor = result1.cursor.as_deref();
        let result2 = client.list_tournaments(&mut session, None, None, None, None, Some(1), cursor).await.unwrap();
        println!("{:?}", result2);
...
fn test_list_tournament_records() {
    ...
    // block_on(async {
    //     let (client, mut session) = helpers::authenticated_client("tournamentclient1").await;
    //     let (client2, mut session2) = helpers::authenticated_client("tournamentclient2").await;
    //     client.join_tournament(&mut session, "example-tournament").await.unwrap();
